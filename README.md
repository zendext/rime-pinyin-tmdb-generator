...
SQLite store 默认路径按媒体类型分开，通常不需要配置：

- TV：`~/.local/state/rime-pinyin-tmdb-generator/series-full.sqlite`
- 电影：`~/.local/state/rime-pinyin-tmdb-generator/movies-full.sqlite`
...
`status` 读取 TV store：`series-full.sqlite`。输出里会包含最近一次抓取/生成时间、词条数量、store 中的剧集数量，以及 TV export 的 date、cursor 和 completed 状态。

首次使用时，建议先手动运行 `generate`，确认命令成功返回，再查看 `status`。当输出里出现下面这一行时，表示当前 TV bootstrap 已完成，可以开启定时任务：
...
- `zh-CN`：TV 生成 `tmdb_full_hans.dict.yaml`
- `zh-TW` / `zh-HK`：TV 生成 `tmdb_full_hant.dict.yaml`
- 电影会额外生成 `tmdb_movie_hans.dict.yaml`，配置 `zh-TW` / `zh-HK` 时额外生成 `tmdb_movie_hant.dict.yaml`

文件名会按 `languages` 自动派生。`hans` 为简体，来自 `zh-CN`；`hant` 为繁体，来自 `zh-TW` / `zh-HK`。没有配置繁体语言时，不会生成 `_hant` 词典。

TV 对应引入 `tmdb_full_hans` 或 `tmdb_full_hant`。电影词库对应引入 `tmdb_movie_hans` 或 `tmdb_movie_hant`。
...
无论在哪个平台，实际输出都会在同目录下按语言生成，例如只配置 `languages = ["zh-CN"]` 时，会生成 `tmdb_full_hans.dict.yaml` 和 `tmdb_movie_hans.dict.yaml`。
...
TMDb API 可以免费用于非商业用途，但需要遵守 TMDb API Terms，包括标注 TMDb 来源、不得商业使用、不得长期缓存或再分发派生数据。这个工具只在用户本地生成词典；请不要把真实生成的 `tmdb_full_hans.dict.yaml`、`tmdb_full_hant.dict.yaml`、`tmdb_movie_hans.dict.yaml` 或 `tmdb_movie_hant.dict.yaml` 作为公开发布文件。
